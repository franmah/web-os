...
export const moveWindow = (event: any, options: WindowState): WindowState => {
	...
	const changeX = isMouseLeftOfScreen(mouseX) || isMouseRightOfScreen(mouseX) ? 0 : mouseX - options.previousClientX;

	const changeY = mouseY - options.previousClientY;
	const newYPosition = isMouseOverTopOfScreen(mouseY) ? 0 : options.top + changeY; // Force position to be at the top to avoid window slowly going down because of lag

	const showMaximizePlacehodler = isMouseOverTopOfScreen(mouseY)
		? MaximizePlaceholderDirection.Full
		: isMouseLeftOfScreen(mouseX)
		? MaximizePlaceholderDirection.Left
		: isMouseRightOfScreen(mouseX)
		? MaximizePlaceholderDirection.Right
