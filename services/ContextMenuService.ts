...
export const correctRootContextMenuPosition = (params: ContextMenuParams): ContextMenuParams => {
	...
	const left = params.left + width > window.innerWidth ? params.left - width : params.left;
	const top =
		params.top + params.commands?.length * CONTEXT_MENU_ITEM_HEIGHT > window.innerHeight - CONTEXT_MENU_HEIGHT_OFFSET
...
export const correctSubMenuLeftPosition = (subMenuLeft: number, subMenuWidth: number, absoluteLeft: number): number => {
	...
	return futureSubMenuRight > window.innerWidth ? subMenuLeft - subMenuWidth : subMenuLeft;
};
