...
export const StyledWindowBorder = styled.div<{ resizing: boolean; zIndex: number }>`
	...
	.top:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'n-resize')};
	}
	.topLeft:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'se-resize')};
	}
	.topRight:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'ne-resize')};
	}
	.bottom:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'n-resize')};
	}
	...
	.leftCenterMiddle:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'e-resize')};
	}
	.leftCenterBottom:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'ne-resize')};
	}
	.rightCenterTop:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'ne-resize')};
	}
	.rightCenterMiddle:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'e-resize')};
	}
	.rightCenterBottom:hover {
		cursor: ${({ resizing }) => (resizing ? 'inherit' : 'se-resize')};
	}
